package com.ads.countriesfooddietreportTestHeapSortQuickSortThreePartitionCountries;
...
class DietData
{
	...
	public void displaySortedCountriesByFruitShare()
	{
		Comparator<CountryDietData> comparator = (c1, c2) ->
										Double.compare(c1.getShare("FRUITS"), c2.getShare("FRUITS"));
		
		List<CountryDietData> countryList = Arrays.stream(this.countries)
												.filter(Objects::nonNull)
												.collect(Collectors.toList());
		
		buildMaxHeap(countryList, comparator);
		System.out.println("Sort countries by the percentage share of fruits : ");
		
		for(int i = 0; i < countryList.size(); i++)
		{
			CountryDietData max = heapExtractMax(countryList, comparator);
			System.out.println(max.getCountry() + ": " + max.getShare("FRUITS")*100 + "%");
		}
	}
	...
	private <T> void maxHeapify(List<T> list, int i, Comparator<? super T> comparator )
	{
		int l = left(i);
		int r = right(i);
		int  max;
		...
		if(r <= heapSize && comparator.compare(list.get(r), list.get(max)) > 0)
			max = r;
		if(max != i)
		{
			swap(list, i, max);
			maxHeapify(list, max, comparator);
		}
	}
	
	private <T> void buildMaxHeap(List<T> list, Comparator<? super T> comparator)
	{
		heapSize = list.size();
		for(int i = list.size()/2-1; i >= 0; i--)
			maxHeapify(list,i,comparator);
	}
	
	private <T> T heapExtractMax(List<T> list, Comparator<? super T> comparator)
	{
		if(heapSize < 1)
	...
	private int[] partition(List<Map.Entry<Integer, Double>> input, int left, int right)
	{
		...
		swap(input,e+1,right);
		
		equalIndexPair[0] = i;
		equalIndexPair[1] = e+1;
